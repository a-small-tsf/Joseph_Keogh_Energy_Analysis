...
### Read through the pnodes to check the uniqueness of names
```{r}
pnode_summary <- pnodes %>%
  select(PNODENAME)

pnode_distinct <- distinct(pnode_summary, .keep_all = FALSE)

pnode_distinct

nrow(pnode_distinct) - nrow(pnode_summary)
```
...
for (pnode in pnodes$PNODEID){
  ...
  "Node Index we are looking at"
  print(count)
  "Total rows found so far"
  print(nrow(response))
